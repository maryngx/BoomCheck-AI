const {
  ...
  extractWithGeminiAI,
} = require("../ai/nlp/nerExtractor"); // extractWithOpenAI
...
exports.handleFileUpload = async (req, res) => {
  try {
    ...
    const aiResults = await extractWithGeminiAI(
      text,
    ...
    const uniqueChemicals = Array.from(
      new Set([...aiResults, ...basicMatched]),
    );
    ...
    res.json({
      ...
      details: {
        ...
        fromFallback: finalChemicals.filter(
          (c) =>
            basicMatched.includes(c.toLowerCase()) &&
            !aiResults.includes(c.toLowerCase()),
        ),
      },
    });
